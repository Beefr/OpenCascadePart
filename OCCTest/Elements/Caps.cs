...
namespace OCCTest.Elements
{
    ...
    public class Caps: BasicElement
    {
        ...
        public Caps(double wallThickness, double diameter, double crownRadius, double knucleRadius, double totalHeight, double useless1, double useless2, string useless3, bool useless4)
        {
            //  can't work if P == O
            ...
            gp_Pnt A = new gp_Pnt(wallThickness, 0, 0); // point à droite de la base
            gp_Pnt B = new gp_Pnt(0, 0, 0); // point à gauche de la base
            gp_Pnt C = new gp_Pnt(0, totalHeight, 0); // debut de l'arc exterieur
            gp_Pnt H = new gp_Pnt(wallThickness, totalHeight, 0); // debut de l'arc intérieur
            gp_Pnt P = new gp_Pnt(wallThickness + crownRadius, totalHeight, 0); // centre du petit cercle formant l'arc
            gp_Pnt O = new gp_Pnt(wallThickness + knucleRadius, totalHeight, 0); // centre du grand cercle formant le capot
            gp_Pnt a1 = new gp_Pnt(crownRadius * Math.Cos(3*Math.PI / 4) + wallThickness + crownRadius, crownRadius * Math.Sin(3*Math.PI / 4)+ totalHeight, 0); // point de l'arc de cercle intérieur 
            gp_Pnt a2 = new gp_Pnt((crownRadius + wallThickness) * Math.Cos(3*Math.PI / 4) + wallThickness + crownRadius, (crownRadius + wallThickness) * Math.Sin(3*Math.PI / 4) + totalHeight, 0); // point de l'arc de cercle extérieur 
            ...
            gp_Pnt E = new gp_Pnt(wallThickness + knucleRadius, totalHeight, 0); // haut du capot
            gp_Pnt F = new gp_Pnt(wallThickness + knucleRadius, totalHeight - wallThickness, 0); // haut du capot mais côté intérieur
            ...
            gp_Pnt G = new gp_Pnt(P.X(), crownRadius * Math.Sin(Math.PI / 2) + totalHeight, 0); // point de l'arc de cercle intérieur 
            gp_Pnt D = new gp_Pnt(G.X(), G.Y()+ wallThickness, 0); // point de l'arc de cercle extérieur 
            ...
            // rotation du wire
            gp_Ax1 Axis = new gp_Ax1(new gp_Pnt(wallThickness + knucleRadius, 0,0), new gp_Dir(0,1,0)); // origine 0,0,0 avec dir 0,1,0 
